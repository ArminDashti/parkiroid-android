...
Step-by-step guide for operating Parkiroid. No technical setup knowledge required beyond having a server address and API key from your administrator.
...
2. The **Parkiroid app** installed on that phone
3. A **Parkiroid server** running and reachable from the phone (usually on your network or a hosted URL)
...
Open Parkiroid. The main screen shows monitoring controls and camera status.
...
3. Confirm the status shows **Running in background** and the *Parkiroid running* notification appears.
