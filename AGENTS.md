...
- Prefer a dedicated Settings screen for server address, API key, and object detection mode rather than inline configuration on the main screen
- Do not commit git changes unless explicitly asked

## Learned Workspace Facts

- Parkiroid is a Kotlin Android app for background parked-vehicle monitoring (rear-camera photos, battery telemetry, optional on-device object detection)
...
- Settings persist in DataStore: server URL, API key (default `parkiroid-dev-key`), object detection mode, and capture period (default 15 s)
